...
var path = require("path"), _defineProperty = require("@babel/runtime/helpers/defineProperty"), fields = require("@keystonejs/fields"), adapterMongoose = require("@keystonejs/adapter-mongoose"), adapterKnex = require("@keystonejs/adapter-knex"), adapterPrisma = require("@keystonejs/adapter-prisma");
...
var path__default = _interopDefault(path), _defineProperty__default = _interopDefault(_defineProperty);
...
class KnexMongoIdInterface extends adapterKnex.KnexFieldAdapter {
  ...
  setupHooks({addPreSaveHook: addPreSaveHook, addPostReadHook: addPostReadHook}) {
    addPreSaveHook(item => {
      ...
      if (item[this.path]) {
        if ("string" != typeof item[this.path] || !validator(item[this.path])) throw new Error(`Invalid MongoID value given for '${this.path}'`);
...
const pkgDir = path__default.default.dirname(require.resolve("@keystonejs/fields-mongoid/package.json")), MongoId = {
  type: "MongoId",
