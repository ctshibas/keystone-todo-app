...
var _defineProperty = require("@babel/runtime/helpers/defineProperty"), _objectWithoutProperties = require("@babel/runtime/helpers/objectWithoutProperties"), fields = require("@keystonejs/fields"), adapterKnex = require("@keystonejs/adapter-knex"), adapterPrisma = require("@keystonejs/adapter-prisma");
...
var _defineProperty__default = _interopDefault(_defineProperty), _objectWithoutProperties__default = _interopDefault(_objectWithoutProperties);
...
class AutoIncrementImplementation extends fields.Implementation {
  constructor(path, _ref = {}, context = {}) {
    let {gqlType: gqlType, isUnique: isUnique = !0, access: access = {}} = _ref, config = _objectWithoutProperties__default.default(_ref, [ "gqlType", "isUnique", "access" ]);
...
class PrismaAutoIncrementInterface extends adapterPrisma.PrismaFieldAdapter {
  constructor() {
    super(...arguments), this.isUnique = void 0 === this.config.isUnique || !!this.config.isUnique, 
    this.isIndexed = !!this.config.isIndexed && !this.config.isUnique;
  }
  ...
  getQueryConditions(dbPath) {
    return _objectSpread(_objectSpread(_objectSpread({}, this.equalityConditions(dbPath, x => Number(x) || -1)), this.orderingConditions(dbPath, x => Number(x) || -1)), this.inConditions(dbPath, x => x.map(xx => Number(xx) || -1)));
  }
}
