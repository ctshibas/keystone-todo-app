...
function sdlArray(schema, options) {
    ...
    var types = objectValues(typeMap).sort(function (type1, type2) {
        ...
        return __assign(__assign(__assign({}, type), stack_1.serialize(schema, type)), { instanceOf: getTypeInstance(type) });
    });
    return types;
}
...
function addLineBreaks(sdl, commentsDisabled) {
    ...
    var withLineBreaks = breakBrackets.replace(/^(?:scalar )\w+/gm, '$&\r\n');
    if (commentsDisabled) {
